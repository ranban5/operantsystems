...
indexApp.controller("indexController", function($scope, $http, $window) {
    ...
    $scope.urlName = "https://www.b-matrix.com";
    webSiteName = $scope.urlName;
    $scope.urlNameForMobile = "https://www.b-matrix.com/mobile/Home.aspx/Login";

    var sendRequestURL = "/html/sendMailHomePage.asmx/sendUpdateRequest";
    var loginURL = "/html/getSharedInfo.asmx/login";

    var imgIntervalTime = 4000;
  ...
  //alert(screen.width)
  //Related to 1920 * 1080
/*  if (screen.height >= 800 && screen.height < 900 ){
    $(".rowSimple1").addClass("hide");
    $(".rowSimple2").addClass("hide");
  }
  else if (screen.height >= 900 && screen.height < 1050){
    $(".rowSimple1").addClass("hide");
  }
*/
  ...
  function showSlides(clsContainer, clsSlide, clsDot) {
    ...
    //setTimeout( function(){return showSlides(clsContainer, clsSlide, clsDot)}, 2000); // Change image every 2 seconds
  }
  ...
  //Login button re-direct to home page of site
    $scope.loginPage = function() {
      location.href = $scope.urlName + "?login=1";
      return;
    ...
    $scope.loginPage_CheckUserIdPassword = function() {
        ...
        callingWS("login", jsonStr)
            .success(function(responseData) {
                var result = JSON.parse(responseData.d);
                ...
                if (result.length == 0)
                    ...
                    $.cookie("parentDBName", result[0].parentDBName, {expires: 7, path: '/'});
                        ...
                        if (result[0].roleName !== "Director"){
                          ...
                          threeLevelOrg = "AperatureEducation|Operantsystems|BehaviorConceptsInc|HillsidePublicSchools|PositiveInterventionPlus|AcronABAUK|DevereuxRedHook|GrainofSandAcademy|Amigo|ShemaKoleinu|Springbrook";
                        }

                        if (threeLevelOrg.indexOf(result[0].dbName)>=0){
    ...
    $scope.availableLink = function(link) {
        if (link == "ios_sb_ipad") {
          ...
          location.href = "https://www.amazon.com/gp/product/B08LGQMF5N";
        }
      ...
      if ( deviceType === 'mobile') {
        ...
        if (localStorage.getItem("localEmailDetails") != null) {
            $.each(lstEmail, function(aryIdx, aryVal) {
                console.log(aryVal.emailId);
                if (aryVal.emailId == $("#cf-email").val()
                    && aryVal.senderName == $("#cf-name").val()
                    && aryVal.senderPhone == $("#cf-phone").val()) {
                    flg = true;
                    cntSender = aryVal.senderCount;
                    return false;
                }
